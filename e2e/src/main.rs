...
use crypto::match_token::{self, EdDSAPublicKey};
use crypto::payload::{CHALLENGE_IV_LEN, CHALLENGE_KEY_LEN, MatchInputs, decode_outcome};
use crypto::sealed_channel::{CHANNEL_VERSION, SealedResponse, UnwrapErr};
...
async fn main() -> Result<()> {
    ...
    let outcome = decode_outcome(&sealed_outcome)
        .map_err(|error| anyhow!("failed to decode the sealed outcome: {error:?}"))?;

    // The cleartext class is the host's hint; the sealed outcome is the authority.
    ensure!(
        response.outcome == MatchOutcome::Statement,
        "enclave reported {:?}, sealed outcome {:?}",
        response.outcome,
        outcome
    );
    let token = outcome.map_err(|reason| anyhow!("match was rejected: {reason:?}"))?;
