...
impl Environment {
    ...
    /// Returns the allowlisted challenge-image locations, as `host/path-prefix` entries.
    ///
    /// # Panics
    ///
    /// Panics when `CHALLENGE_IMAGE_ALLOWLIST` is unset or empty. There is no safe default: a
    /// fetcher with nothing pinned would retrieve from anywhere a caller names, so refusing to
    /// start is the only correct reading of a missing value.
    #[must_use]
